...
public class SwiftQrScannerAlternativePlugin: NSObject, FlutterPlugin, ScanBarcodeDelegate,FlutterStreamHandler {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let channel = FlutterMethodChannel(name: "qr_scanner_alternative", binaryMessenger: registrar.messenger())
        let instance = SwiftQrScannerAlternativePlugin()
        registrar.addMethodCallDelegate(instance, channel: channel)
        let eventChannel=FlutterEventChannel(name: "qr_scanner_alternative_receiver", binaryMessenger: registrar.messenger())
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        SwiftQrScannerAlternativePlugin.barcodeStream = events
        return nil
    }
    
    public func onCancel(withArguments arguments: Any?) -> FlutterError? {
        SwiftQrScannerAlternativePlugin.barcodeStream=nil
        return nil
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let colorCode = args["lineColor"] as? String{
            SwiftQrScannerAlternativePlugin.lineColor = colorCode
        }else {
            SwiftQrScannerAlternativePlugin.lineColor = "#ff6666"
        }
        if let buttonText = args["cancelButtonText"] as? String{
            SwiftQrScannerAlternativePlugin.cancelButtonText = buttonText
        }else {
            SwiftQrScannerAlternativePlugin.cancelButtonText = "Cancel"
        }
        if let flashStatus = args["isShowFlashIcon"] as? Bool{
            SwiftQrScannerAlternativePlugin.isShowFlashIcon = flashStatus
        }else {
            SwiftQrScannerAlternativePlugin.isShowFlashIcon = false
        }
        if let isContinuousScan = args["isContinuousScan"] as? Bool{
            SwiftQrScannerAlternativePlugin.isContinuousScan = isContinuousScan
        }else {
            SwiftQrScannerAlternativePlugin.isContinuousScan = false
        }
        
        if let scanModeReceived = args["scanMode"] as? Int {
            if scanModeReceived == ScanMode.DEFAULT.index {
                SwiftQrScannerAlternativePlugin.scanMode = ScanMode.QR.index
            }else{
                SwiftQrScannerAlternativePlugin.scanMode = scanModeReceived
            }
        }else{
            SwiftQrScannerAlternativePlugin.scanMode = ScanMode.QR.index
        }
        ...
        if checkCameraAvailability(){
            if checkForCameraPermission() {
                SwiftQrScannerAlternativePlugin.viewController.present(controller
                                                                        , animated: true) {
                ...
                AVCaptureDevice.requestAccess(for: .video) { success in
                    DispatchQueue.main.async {
                        if success {
                            ...
                            SwiftQrScannerAlternativePlugin.viewController.present(alert, animated: true)
                        }
                    }
                }}
        }else {
    ...
    func showAlertDialog(title:String,message:String){
        ...
        SwiftQrScannerAlternativePlugin.viewController.present(alertController, animated: true, completion: nil)
    }
}
...
class BarcodeScannerViewController: UIViewController {
    ...
    private lazy var flashIcon : UIButton! = {
        ...
        flashButton.setImage(UIImage(named: "ic_flash_off", in: Bundle(for: SwiftQrScannerAlternativePlugin.self), compatibleWith: nil),for:.normal)
    ...
    private lazy var switchCameraButton : UIButton! = {
        ...
        button.setImage(UIImage(named: "ic_switch_camera", in: Bundle(for: SwiftQrScannerAlternativePlugin.self), compatibleWith: nil),for: .normal)
    ...
    public lazy var cancelButton: UIButton! = {
        let view = UIButton()
        view.setTitle(SwiftQrScannerAlternativePlugin.cancelButtonText, for: .normal)
        view.translatesAutoresizingMaskIntoConstraints = false
    ...
    func initUIComponents(){
        if isOrientationPortrait {
            screenHeight = (CGFloat)((SwiftQrScannerAlternativePlugin.scanMode == ScanMode.QR.index) ? (screenSize.width * 0.8) : (screenSize.width * 0.5))
            
        } else {
            screenHeight = (CGFloat)((SwiftQrScannerAlternativePlugin.scanMode == ScanMode.QR.index) ? (screenSize.height * 0.6) : (screenSize.height * 0.5))
        }
    ...
    func drawUIOverlays(withCompletion processCompletionCallback: () -> Void){
        ...
        if let qrCodeFrameView = qrCodeFrameView {
            ...
            if(!SwiftQrScannerAlternativePlugin.isShowFlashIcon){
    ...
    private func flashIconOff() {
        flashIcon.setImage(UIImage(named: "ic_flash_off", in: Bundle(for: SwiftQrScannerAlternativePlugin.self), compatibleWith: nil),for:.normal)
    }
    
    private func flashIconOn() {
        flashIcon.setImage(UIImage(named: "ic_flash_on", in: Bundle(for: SwiftQrScannerAlternativePlugin.self), compatibleWith: nil),for:.normal)
    }
    ...
    @IBAction private func cancelButtonClicked() {
        if SwiftQrScannerAlternativePlugin.isContinuousScan{
            self.dismiss(animated: true, completion: {
                SwiftQrScannerAlternativePlugin.onBarcodeScanReceiver(barcode: "-1")
            })
        }else{
    ...
    private func drawLine() {
        ...
        scanLine.backgroundColor = hexStringToUIColor(hex: SwiftQrScannerAlternativePlugin.lineColor)
        ...
        if SwiftQrScannerAlternativePlugin.scanMode == ScanMode.QR.index {
    ...
    public func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if supportedCodeTypes.contains(metadataObj.type) {
            ...
            if metadataObj.stringValue != nil {
                if(SwiftQrScannerAlternativePlugin.isContinuousScan){
                    SwiftQrScannerAlternativePlugin.onBarcodeScanReceiver(barcode: metadataObj.stringValue!)
                }else{
...
extension BarcodeScannerViewController{
    ...
    func updateUIAfterRotation(){
        DispatchQueue.main.async {
            ...
            if UIDevice.current.orientation == .portrait || UIDevice.current.orientation == .portraitUpsideDown {
                self.screenHeight = (CGFloat)((SwiftQrScannerAlternativePlugin.scanMode == ScanMode.QR.index) ? (self.screenSize.width * 0.8) : (self.screenSize.width * 0.5))
                
            } else {
                self.screenHeight = (CGFloat)((SwiftQrScannerAlternativePlugin.scanMode == ScanMode.QR.index) ? (self.screenSize.height * 0.6) : (self.screenSize.height * 0.5))
            }
